...
class TaskNotificationScheduler {
  ...
  private async sendTaskReminderEmail(task: Task, reminderType: string) {
    try {
      ...
      if (!transporter) {
        console.log('[TaskScheduler] No email service configured. Logging notification instead.');
      ...
      const subject = `⏰ Task Reminder: Due in ${reminderType}`;
      const htmlContent = `
...
This reminder was automatically generated by FlowHub Command Center.
      ...
      console.log(`[TaskScheduler] Sending task reminder email: ${subject}`);
      ...
      console.log(`[TaskScheduler] Task reminder email sent successfully: ${subject}`, result.messageId);
      return true;
    } catch (error) {
      console.error(`[TaskScheduler] Email sending failed for task reminder: ${task.title}`);
